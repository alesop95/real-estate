...
Da qui le due strade, e la seconda non è stata presa. Lasciare così, e il grafo è la stella di sedici nodi che si vede ora, corretta ma povera: è anche la ragione per cui i nodi orfani sono nascosti, perché mostrarli aggiunge trentatré punti sospesi e nessuna informazione che la cartella non dia già meglio. Oppure convertire i due indici in collegamenti veri, che è una modifica al contenuto e non alla vista, quindi fuori dal criterio dichiarato in questa scheda, e che però avrebbe un valore indipendente dal vault, perché un indice con i nomi cliccabili è migliore anche su GitHub e nell'editor. La conversione ha senso su `docs/README.md`, dove i quindici documenti sono citati in prosa e in tabella; non su `CLAUDE.md`, dove i nomi stanno dentro blocchi preformattati e un collegamento in un blocco preformattato resta testo per costruzione. Chi la fa ottiene il secondo hub, il grappolo delle guide e quello delle schede di dominio, e `fonti.md` al grado che gli spetta.
