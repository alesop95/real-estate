...
### La simulazione ha ora tre implementazioni, e il modo in cui si tengono insieme

Fino all'8 settembre 2026 la simulazione esisteva soltanto come formule del foglio Rischio, e questo la rendeva l'unica parte del modello non verificabile se non guardandola: le sue proprietà erano dimostrate a mano e misurate dentro Excel, mentre ogni altra grandezza aveva un motore Python accanto a confermarla. Il 9 settembre è stata scritta in Python, in `src/immobiliare/rischio.py`, e portata in TypeScript per il browser, in `app/src/motore/rischio.ts`. Le tre implementazioni si tengono insieme in due punti diversi, e la distinzione conta.

Fra il foglio e Python il confronto è diretto e passa dalle estrazioni. Il modulo Python le rigenera con lo stesso seme e lo stesso ordine di chiamata del generatore del workbook, quindi i due lavorano sugli stessi mille numeri: le prime cinque righe del foglio nascosto coincidono cifra per cifra, e sulle diciannove colonne calcolate lo scarto massimo è dell'ordine di dieci alla meno dodici. Sui numeri che il foglio mostra, cioè i quindici percentili, le quattro probabilità e le ventiquattro celle del tornado, lo scarto massimo è 1,5 su dieci alla tredicesima. Quei valori sono ora congelati in `tests/test_rischio.py`, così che la coincidenza non resti la misura di un giorno.

Fra Python e TypeScript il confronto passa dai vettori, come per il resto del motore, ma con una differenza che riguarda la natura della funzione: la simulazione non dipende dai soli input, dipende anche dalle estrazioni, e due linguaggi non condividono il generatore pseudocasuale. I vettori portano quindi anche il campione di estrazioni con cui sono stati prodotti, e la suite TypeScript lo usa come ingresso invece di estrarre per conto proprio. Le estrazioni diventano un ingresso e non un effetto interno, che è la sola forma in cui una funzione con una fonte di casualità si può confrontare bit per bit.

Il porto ha fatto emergere due cose che il foglio non diceva, e vanno registrate qui perché riguardano il modello e non il codice. La prima è che la simulazione tassa il ricavo effettivo con la cedolare secca sul canone libero anche quando il regime scelto è un altro, perché la formula del foglio nascosto cita quella cella: nei due moduli l'aliquota è un parametro con lo stesso valore predefinito, quindi il confronto resta esatto e l'applicazione potrà passare l'aliquota del regime scelto. La seconda è che la formula chiusa del debito residuo divide per la differenza fra due montanti, che a tasso nullo è zero: nel foglio quel caso produrrebbe un errore di divisione, mentre nei due moduli è trattato per quello che è, cioè il capitale rimborsato in parti uguali.
