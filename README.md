...
Il perimetro di ciò che esce dalla macchina è cambiato il 4 settembre 2026, quando il progetto ha deciso di diventare anche un'applicazione web autenticata, con lo scopo di usarlo dal browser e di venderlo a un'agenzia immobiliare. La distinzione che vale da allora è questa: possono stare in rete il modello di calcolo, che è pubblico per costruzione e documentato qui dentro, e i dati che un utente inserisce nel proprio account, protetti da autenticazione e isolati per organizzazione. Non ci vanno, e non ci vanno nemmeno per comodità di prova, il registro degli annunci con i prezzi obiettivo, il materiale sotto `_notes/`, i recapiti di terzi e i documenti di una trattativa reale. Resta esclusa la pubblicazione della documentazione su piattaforme di terzi, che è una scelta separata dal servire un'applicazione. La direzione, le piattaforme misurate e le alternative scartate stanno in [`docs/architettura-web.md`](<docs/architettura-web.md>).
