...
Il workbook ha ventun fogli e si apre sul Cruscotto, che raccoglie i cinque numeri su cui si decide. Quelli in cui si lavora sono Immobile, Mutuo, Locazione, Cash flow e Annunci, dove le celle gialle sono gli input. Quelli che si leggono sono Confronto immobili, Metriche, Confronto affitto, Scenari e Rischio. Il Simulatore mutuo sta a se': serve a provare rimborsi volontari e rialzi di tasso senza toccare l'analisi principale. Quelli che si consultano sono Guida, Parametri, Checklist, Dossier tecnico e Fonti, piu' il piano di ammortamento rata per rata. Il foglio Asta serve solo a chi guarda le vendite giudiziarie, e sta a se' perche' le regole sono altre.
...
Il foglio Asta modella l'acquisto in una vendita giudiziaria, che non e' una compravendita ordinaria con un prezzo piu' basso. Non c'e' provvigione di agenzia, ed e' il solo risparmio strutturale; in cambio non esiste garanzia per i vizi ai sensi dell'articolo 2922 del codice civile, il debitore e i familiari conviventi restano nel possesso fino al decreto di trasferimento, una locazione con data certa anteriore al pignoramento e' opponibile a chi aggiudica, e chi non versa il saldo nel termine decade e perde la cauzione a titolo di multa. Il foglio mette in conto tutto questo e arriva a due numeri: lo sconto effettivo sul valore di mercato, da confrontare con una soglia che rappresenta il prezzo di quei rischi, e il prezzo massimo a cui fermarsi in gara. La materia e' spiegata in `docs/aste-immobiliari.md`.
...
Chi parte da zero, senza Python installato e senza sapere quali documenti servano, legge `docs/da-zero.md`: sette passi dall'ambiente vuoto alla prima valutazione completa. Chi vuole solo usare il file legge `docs/guida-non-tecnica.md`, che accompagna foglio per foglio spiegando ogni voce in linguaggio comune. Chi interviene sul modello legge `docs/guida-tecnica(catena-calcolo-e-normativa).md`, che riporta architettura, catena di calcolo e il riferimento di ogni voce con formula, nome definito e norma di riferimento.

Chi compra insieme a qualcun altro ha un problema in piu': spiegare le conclusioni a una persona che non ha il progetto sulla macchina e non lancera' mai un comando. Per quello c'e' `docs/guida-per-il-socio.md`, che si legge da sola e si puo' mandare come file: copre le tre domande a cui lo strumento risponde, il percorso in cinque passi, i cinque colori delle celle, la tabella dei venti fogli con quali riguardano chi, e tutte e cinquantuno le celle di input spiegate una per una con che cosa scrivere e perche' serve. Chiude con un giro completo su un immobile reale, numeri compresi, e con i limiti dichiarati. Le voci non sono scritte a memoria ma estratte dal workbook generato, leggendo etichetta, colore di riempimento e nota di ogni cella di input: e' completa per costruzione e non per attenzione.

Chi vuole sapere che matematica c'e' sotto, e non fidarsi, legge `docs/matematica/matematica-finanziaria.tex`, che si compila in un PDF di trentadue pagine con `scripts/build.ps1 -Main docs\matematica\matematica-finanziaria.tex`. E' la trattazione completa: capitalizzazione e sconto, rendite costanti e crescenti con le derivazioni, l'equazione di Fisher e l'errore della sottrazione, la base imponibile come funzione a tratti, il costo totale come funzione affine del prezzo, la derivazione della rata alla francese e del debito residuo in forma chiusa, il percorso del tasso a gradini, i cinque indicatori con la formula della leva derivata, il tasso interno con esistenza, unicita' e metodo numerico, l'inversione che da' il prezzo massimo sostenibile, sensibilita' ed elasticita', la simulazione con la correzione sulla radice degli anni, l'effetto dell'inflazione componente per componente, il confronto comprare-affittare e la graduatoria. Chiude con la tavola che lega ogni simbolo alla cella del workbook e alla funzione Python, il caso di riferimento svolto con tutti i valori verificati, e l'elenco dei limiti dichiarati.
