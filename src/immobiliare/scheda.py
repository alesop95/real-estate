...
numeri sono il costo reale dell'operazione, che non è il prezzo, il rendimento
netto reale, il prezzo massimo che l'immobile giustifica ai propri criteri, e lo
sconto che ne consegue. L'elenco è quello dei dati che mancano, perché la
telefonata serve anche a chiederli.

Il workbook risponde alle stesse domande e non serve a questo: è un file di
ventun fogli che si consulta da fermi, e in trattativa non si apre. La scheda è
l'estrazione di ciò che si porta con sé.

Tre scelte di progetto vanno dichiarate.

La prima è che la scheda calcola con il motore Python e non legge il workbook:
sono due implementazioni della stessa matematica, e usare la prima rende la
scheda un terzo riscontro invece di una copia.

La seconda è che i dati mancanti non vengono taciuti né sostituiti con valori
...
versione lo stampava, e il risultato è stato istruttivo: su un immobile senza
...
La terza riguarda la separazione fra testo e marcatura, ed è nata da un difetto
...
rovesciata. La regola che ne discende è che il testo proveniente dai dati si
...
def _esc(testo) -> str:
    ...
    Si applica a tutto ciò che arriva da un campo compilato a mano e mai a una
    stringa che contiene marcatura: sfuggire la marcatura la stampa alla lettera,
    ed è esattamente il difetto che questa separazione previene.
    """
    if testo is None:
        return ""
...
def _riga(etichetta: str, valore: str, nota: str = "", grassetto: bool = False) -> str:
    ...
    L'etichetta e la nota sono testo e vengono sfuggite; il valore è marcatura
    prodotta dalle funzioni di formato di questo modulo. Il grassetto è un
    parametro e non un pezzo di stringa, che è la ragione per cui questa
...
def _tabella(righe: list[str]) -> list[str]:
    ...
    dalla pagina: un difetto che non compare nel sorgente né fra gli avvisi del
    compilatore, e che si vede solo guardando il PDF.
    """
    # La terza colonna è testo esplicativo e va in corpo più piccolo: è
    # l'elemento che manda a capo di più, quindi è quello che decide se la
    ...
    # La chiusura di paragrafo non è cosmetica: senza, il titolo della sezione
    # seguente si aggiunge al paragrafo della tabella, compare a destra di essa
    # e viene sillabato a metà. Sul PDF compilato si leggeva "Il costo rea-"
    # accanto alla prima tabella e "le, che non è il prezzo" sulla riga sotto:
...
def costruisci(
    ...
    è una scelta di chi valuta. Il valore predefinito dell'IMU è l'aliquota
    base di legge, e la scheda lo dichiara come tale invece di lasciarlo passare
    per un dato: è lo stesso presidio dei controlli del Cruscotto.
    """
    ...
    # Tre condizioni che decidono che cosa la scheda può dire, separate perché
    # bloccano cose diverse: senza prezzo non esiste operazione, senza canone non
    # esistono rendimenti né prezzo massimo, senza rendita il prezzo-valore non
    ...
    if mancanti:
        avviso = (
            ...
            "I numeri sono calcolati su ciò che c'è e vanno letti come provvisori. "
        )
        if not ha_rendita:
            avviso += (
                "In particolare manca la rendita catastale, quindi l'opzione prezzo-valore non "
                "si applica e le imposte risultano calcolate sul prezzo intero: è la voce che, "
                "arrivando, abbassa di più il costo dell'operazione. "
            )
        if not ha_canone:
            avviso += (
    ...
    if annuncio.prezzo_obiettivo:
        righe.append(_riga("Prezzo obiettivo", _euro(annuncio.prezzo_obiettivo),
                           "è quello usato in tutti i conti di questa scheda"))
    if annuncio.mq:
    ...
    if annuncio.quotazione_omi_min and annuncio.quotazione_omi_max:
        ...
        righe.append(_riga("Scarto sulla media di zona", _perc(annuncio.scarto_su_omi, 1),
                           "sotto la media non è di per sé un affare: una ragione c'è sempre"))
    c.extend(_sezione("Il prezzo, e come sta rispetto alla zona"))
    c.extend(_tabella(righe))

    # ---------------------------------------------------------- il costo reale
    if immobile.venditore_impresa:
        nota_base = "nel regime IVA la base imponibile è il prezzo"
    elif ha_rendita:
        nota_base = "con il prezzo-valore la base è il valore catastale e non il prezzo"
    else:
        nota_base = "senza rendita catastale il prezzo-valore non si applica: la base è il prezzo"
    ...
    righe.append(_riga("Costo totale", _euro(costo.costo_totale),
                       "è il denominatore di ogni rendimento di questa scheda", grassetto=True))
    righe.append(_riga("Cassa necessaria al rogito", _euro(costo.esborso_iniziale),
                       "costo totale meno il mutuo erogato"))
    c.extend(_sezione("Il costo reale, che non è il prezzo"))
    c.extend(_tabella(righe))
    ...
    if not ha_canone:
        ...
        righe = [
            ...
            _riga("Ricavo effettivo", _euro(conto.canone_effettivo),
                  "al netto di un mese di sfitto e dell'accantonamento per morosità"),
            ...
            _riga("Rendimento netto reale", _perc(reale),
                  ...
                  + " per cento: è il rendimento in potere d'acquisto", grassetto=True),
        ...
        if mutuo:
            ...
            righe.append(_riga("Cash on cash", _perc(metriche.cash_on_cash),
                               "sul capitale proprio: con la leva può essere negativo"))
        ...
        righe.append(_riga("Cash flow annuo", _euro(metriche.cash_flow_annuo),
                           "diviso dodici è l'impegno mensile di tasca propria"))
        c.extend(_tabella(righe))
    ...
    if prezzo_massimo is None:
        c.append(r"\colorbox{yellow!30}{\parbox{\dimexpr\linewidth-2\fboxsep}{"
                 r"\textbf{Il numero da portare in trattativa non è calcolabile.}\\[0.3em]"
                 r"Il prezzo massimo sostenibile è il prezzo a cui il rendimento netto raggiunge "
                 r"l'obiettivo, quindi dipende dal canone: senza il canone atteso l'equazione non "
                 r"ha dati, e una cifra stampata qui sarebbe inventata. È la ragione per cui "
                 r"questa casella è vuota invece di contenere un numero.}}")
    else:
        ...
        if prezzo_massimo <= 0:
            testo += (
                r"Il valore risulta negativo, e la lettura corretta non è che si debba chiedere "
                r"più del prezzo: è che a quel rendimento obiettivo l'operazione non è "
                r"giustificabile a nessun prezzo positivo, perché i costi fissi e i costi annui "
                r"che scalano col prezzo eccedono ciò che il canone produce."
            )
        elif sconto < 0:
            testo += (
                r"Rispetto al prezzo usato in questa scheda lo sconto da ottenere è di \textbf{"
                + _euro(abs(sconto)) + r"}, cioè il " + _num(abs(sconto) / prezzo * 100, 1)
                + r" per cento del prezzo."
            )
        else:
            testo += (
                r"Il prezzo trattato è quindi sotto quello che l'immobile giustifica, con un "
    ...
    assunzioni = (
        ...
        + (", che è il valore base di legge e va sostituito con quello della delibera del Comune"
           if abs(imu - P.IMU.aliquota_base) < 1e-12 else "")
        + ("; nessun mutuo, quindi il capitale è interamente proprio" if not mutuo else
           "; mutuo di " + _num(mutuo) + " euro al " + _num(tasso * 100, 2)
           + " per cento su " + str(durata) + " anni")
        + "; un mese di sfitto atteso all'anno e accantonamento per morosità al "
        + _num(P.COSTI.morosita_su_canone * 100) + " per cento; regime di locazione a cedolare secca."
    )
    c.append(r"\par\vspace{0.5em}")
    c.append(r"{\scriptsize " + _esc(assunzioni))
    c.append(
        r"\\[0.4em] Questa scheda è uno strumento di analisi personale e non costituisce "
        r"consulenza fiscale, legale o finanziaria. Prima di firmare qualunque cosa le posizioni "
        r"soggettive vanno confermate da un notaio e da un commercialista, e la conformità "
        ...
        r"foglio Dossier tecnico: una proposta accettata è già un contratto.}"
    )
