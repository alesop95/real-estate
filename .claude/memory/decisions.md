...
## ADR-021, i parametri comunali si raggiungono per collegamento calcolato, e si conservano con la data di lettura

Data: 2026-09-04. Stato: accettata.

Contesto. Due voci del modello non hanno un valore nazionale e vanno lette Comune per Comune: l'aliquota IMU e l'imposta di soggiorno. Per mesi il registro delle fonti le ha dichiarate come lacune con la stessa formula, cioè che l'unica fonte corretta è l'atto del Comune, e la conseguenza pratica era che ogni volta si ripartiva da una ricerca a mano. La domanda posta è stata se non convenisse una tabella per città o provincia da cui pescare il valore ogni volta.

Decisione. Non si conserva una tabella di valori, si conservano due cose distinte. Il collegamento all'atto IMU non si conserva affatto: si calcola, perché l'applicazione del Dipartimento delle finanze accetta il codice catastale del Comune e la sigla della provincia, ed entrambi si leggono dalla fornitura OMI già in cache. Il valore letto in quell'atto si conserva invece in `data/comuni-verifiche.csv`, una riga per Comune, sempre accompagnato dalla data di lettura, e `stato_verifica` traduce quella data in uno di quattro esiti. Per l'imposta di soggiorno, dove un elenco nazionale degli atti non esiste, si conserva anche il collegamento.

Motivazione. I due dati non sono fissi, ed è il punto che rende sbagliata la tabella di valori: l'aliquota IMU si ridelibera ogni anno e l'atto ha efficacia per l'anno se pubblicato entro il 28 ottobre, mentre l'imposta di soggiorno cambia con ogni regolamento nuovo. Una tabella di valori congelati sarebbe quindi un giacimento di numeri scaduti con l'aspetto di numeri verificati, che è la cosa precisa che il registro delle fonti esiste per impedire. Calcolare il collegamento invece di conservarlo evita in più una tabella di ottomila righe da mantenere, e fa sì che la copertura cresca da sola quando si importa la fornitura di una regione nuova.

Conseguenze. Il registro tiene separato sapere dove sta il dato dal sapere il dato, e una riga può portare un collegamento verificato e nessun valore: è il caso di Civitanova, dove la delibera delle tariffe è un PDF non ancora aperto, e il comando continua a dichiarare la voce mancante. Nasce una quarta scadenza ricorrente, dopo il 28 ottobre, quando una lettura provvisoria diventa definitiva. Il modulo non fa rete e resta quindi fuori dalla catena che produce il workbook.

## ADR-022, un'ingestione è atomica, e un filtro che non tiene nulla rifiuta invece di riuscire

Data: 2026-09-04. Stato: accettata.

Contesto. La fornitura OMI chiesta all'area riservata per le Marche conteneva anche il Piemonte: 15.254 quotazioni su 1.180 Comuni estranei alla valutazione, dentro un file che si presentava come l'ambito territoriale richiesto. Per giorni la scheda di stato ha riportato il totale come se fosse tutto marchigiano, e le ricerche per nome rispondevano su Comuni di un'altra regione senza che nulla segnalasse l'anomalia. La prima pulizia è stata fatta a mano sui CSV in cache.

Decisione. Il filtro diventa un'opzione dell'importazione, `--regione`, ripetibile, e l'importazione diventa atomica: i file si estraggono in una cartella di transito, si filtrano lì, e solo alla fine si spostano nella cache. Se il filtro non tiene nessuna riga, l'importazione fallisce, la cache non viene toccata e il messaggio elenca le regioni che il file contiene davvero. L'archivio scaricato non viene modificato in nessun caso.

Motivazione. Una pulizia a mano non è riproducibile e va rifatta a ogni fornitura nuova, quindi prima o poi si dimentica. Sull'atomicità la ragione è che la prima versione filtrava dopo aver copiato: in caso di rifiuto la cache conteneva i file non filtrati mentre il messaggio diceva che non era stata toccata, cioè il programma affermava il falso. Sul rifiuto a scrivere un file vuoto la ragione è il caso probabile, cioè il nome di regione scritto in una forma diversa da quella della fornitura: senza il rifiuto si otterrebbe un'importazione riuscita e una cache senza quotazioni, che è un silenzio plausibile, e in questo progetto un silenzio plausibile è considerato peggiore di un errore rumoroso.

Conseguenze. Chi importa deve dire quale regione vuole, e se non lo dice ottiene il file così com'è, con l'avvertenza ora scritta in tre documenti. Il filtro riferisce quante righe tiene e quante scarta, perché una riduzione silenziosa sarebbe indistinguibile da una fornitura incompleta. Il file riscritto è in UTF-8 anche quando l'originale era nella codifica ANSI di Windows, che il lettore riconosce comunque.

## ADR-023, la correlazione fra le variabili di scenario si dichiara con un fattore comune, non con una matrice

Data: 2026-09-04. Stato: accettata.

Contesto. La simulazione del foglio Rischio estraeva le sue cinque variabili indipendentemente l'una dall'altra e lo dichiarava fra i limiti del modello. L'assunzione sottostimava le code nella direzione che conta: per un acquisto a leva lo scenario che va messo in conto è quello in cui i tassi salgono mentre i prezzi scendono e lo sfitto cresce, cioè le cose cattive arrivano insieme. Il limite era rimasto aperto con la motivazione che una matrice di correlazione richiede di stimare parametri che nessuno possiede, e che introdurla avrebbe sostituito un'assunzione dichiarata con una nascosta.

Decisione. Si introduce un fattore comune di scenario, estratto insieme alle altre variabili e congelato come loro, che entra in ciascuna con il segno del verso in cui quella variabile reagisce a uno scenario favorevole: canone e rivalutazione salgono, sfitto e tasso scendono, la morosità diventa meno probabile. L'intensità è un solo numero, esposto come cella di input del foglio Rischio con valore predefinito del trenta per cento. I pesi con cui il fattore comune e la componente propria si mescolano sono la radice della correlazione e la radice del suo complemento. Sull'evento di morosità, la cui estrazione è una probabilità e non uno scarto, la mescolanza passa per l'inversa della normale e torna con la cumulata.

Motivazione. La forma a fattore comune richiede un parametro invece di una dozzina, e quel parametro resta visibile e modificabile, quindi l'assunzione resta dichiarata: è la condizione che il limite poneva. La scelta dei due pesi come radici discende da due proprietà volute. La prima è che la somma dei loro quadrati fa uno, quindi ogni estrazione efficace resta una normale standard e le incertezze dichiarate in cima al foglio continuano a valere esattamente quelle: introdurre la correlazione non gonfia di nascosto le volatilità, che era il modo in cui questa modifica poteva mentire. La seconda è che il parametro si legge come una correlazione e non come un coefficiente astratto, perché fra due variabili di verso concorde la correlazione risultante è esattamente il valore impostato.

Conseguenze. A correlazione zero la simulazione torna identica a quella indipendente, estrazione per estrazione, e la verifica con Excel misura uno scarto di 2,2 su dieci alla sedicesima, cioè la precisione della macchina: la modifica è quindi verificabile e non opinabile. Al valore predefinito, sul caso precaricato, la mediana del flusso di cassa resta ferma mentre il quinto percentile peggiora di 1.694 euro l'anno. Il foglio nascosto delle estrazioni cresce di sei colonne, cinque delle quali mostrano le estrazioni efficaci, e questo è deliberato: la mescolanza è visibile a chi ispeziona il motore invece di essere sepolta dentro una formula più lunga. Il valore predefinito resta una convenzione dichiarata e non una stima, e il modo corretto di usarlo è muoverlo. Quando il motore verrà portato in TypeScript, la simulazione andrà scritta là per la prima volta e queste proprietà sono la sua specifica.

## ADR-024, la piattaforma si sceglie sul criterio della gratuità strutturale, e quel criterio porta a Cloudflare

Data: 2026-09-04, rivista il 2026-09-04 dopo la seconda ricerca. Stato: accettata.

Contesto. Deciso il passaggio a un'applicazione web autenticata, la prima ricerca aveva scelto Firebase per il minimo attrito: autenticazione gestita, nessuna sospensione per inattività, e un piano gratuito che non richiede alcun metodo di pagamento. Scrivendo i vincoli di quel piano è emerso il suo prezzo vero: su Spark le Cloud Functions non si possono distribuire, quindi niente codice lato server, e con esso niente lavori pianificati, niente segreti, niente archiviazione di file, niente operazioni al di sopra delle regole di sicurezza. La domanda che ne è seguita, cioè se esista un full-stack gratuito senza quei vincoli, ha riaperto la scelta.

Decisione. La piattaforma è Cloudflare: Workers per il codice lato server, D1 come database relazionale, R2 per i file quando serviranno, attivazioni pianificate e segreti dentro la fascia gratuita, Zero Trust Access come autenticazione perimetrale fino a cinquanta utenti, e distribuzione delle risorse statiche nello stesso deploy del Worker. Il criterio che ha deciso è che il piano gratuito non richieda un metodo di pagamento e che il superamento di un limite produca un errore e non un addebito: è una proprietà strutturale, non una promessa da sorvegliare. Firebase resta la scelta corretta per chi accetti l'architettura senza server; Supabase esce per la sospensione dopo una settimana di inattività; Oracle, che è l'unica senza vincoli di piattaforma, esce perché chiede una carta, recupera le istanze inattive con una soglia che il profilo d'uso di questo strumento incontra, e trasforma chi la usa in sistemista; Render esce per il database gratuito che scade in trenta giorni; Vercel per il divieto di uso commerciale sul piano Hobby.

Motivazione. Fra le due candidate senza carta e senza sospensioni, la differenza non è il costo ma il soffitto. Su Firebase la prima cosa che richiedesse un promemoria, una chiave verso un servizio esterno o un pannello che agisce sui dati di un altro utente obbligherebbe a migrare tutto o a pagare, e sarebbe un debito contratto senza saperlo. Su Cloudflare quel soffitto non c'è, al prezzo di scrivere un'interfaccia di programmazione al posto delle regole dichiarative, cioè forse due o tre volte il codice sulla parte dati. Il prezzo è accettabile perché il progetto ha un orizzonte lungo e un'ipotesi di vendita, e non lo sarebbe se l'orizzonte fosse una valutazione sola.

Conseguenze. L'autorizzazione passa da regole dichiarative a codice ordinario, e con essa il modo in cui si sbaglia: una regola dimenticata nega, una rotta che dimentica il controllo concede. Da qui la disciplina, vincolante, che nessuna rotta interroghi il database da sola, che tutte passino da una funzione unica che risolve chiamante, organizzazione e ruolo, e che ciascuna abbia la coppia di test sul caso permesso e su quello negato. Il limite da sorvegliare non è più una quota tecnica ma i cinquanta utenti di Access, che è anche l'unico numero di questo studio preso da fonti di terzi concordanti e non dalla pagina del fornitore: va riverificato prima di impegnarsi.

## ADR-025, lo strumento diventa un prodotto da vendere, e le conseguenze sono tecniche

Data: 2026-09-07. Stato: accettata.

Contesto. Il progetto era nato come strumento personale per una decisione reale, e il passaggio al web era motivato dall'uso. Il 7 settembre 2026 è stato dichiarato il secondo scopo: vendere lo strumento a un'agenzia immobiliare. Non è un'aggiunta di perimetro funzionale, perché il motore di calcolo è lo stesso, ma è un cambio di requisiti non funzionali, e conviene registrarlo prima di scrivere il codice invece di scoprirlo dopo.

Decisione. La valutazione personale resta il banco di prova e non diventa un caso d'uso secondario, perché uno strumento che non serve a chi lo scrive non serve a nessuno. Contemporaneamente ogni scelta tecnica va giudicata anche come scelta di prodotto, con tre requisiti che diventano vincolanti: la fascia gratuita della piattaforma deve ammettere l'uso commerciale, l'organizzazione diventa la radice del modello dei dati dal primo giorno invece di essere una struttura da aggiungere se un giorno servisse, e i dati di un cliente non sono materiale di prova, quindi sviluppo ed esercizio restano ambienti separati.

Motivazione. I tre requisiti hanno lo stesso carattere: sono cose che, se non decise all'inizio, si pagano con una riscrittura. La licenza di una piattaforma non si negozia a posteriori, e la clausola non commerciale di un piano gratuito si scopre quando il prodotto ha già dei clienti. La multiproprietà dei dati aggiunta dopo obbliga a migrare ogni tabella e ogni rotta. La separazione degli ambienti, se non nasce con il progetto, viene violata la prima volta che serve un dato realistico per provare qualcosa.

Conseguenze. Il vincolo di riservatezza in `CLAUDE.md` è stato riscritto invece di essere cancellato, e ora distingue ciò che può stare in rete, cioè il modello e i dati che un utente inserisce nel proprio account, da ciò che non ci va comunque, cioè il registro con i prezzi obiettivo, il materiale sotto `_notes/` e i documenti di trattative reali. La voce della roadmap che prevedeva una versione parallela per l'agente immobiliare non è più facoltativa: è lo scopo, e in quella forma è stata sostituita. Resta aperta, e non è una questione tecnica, la scelta della licenza con cui il prodotto viene venduto: il repository è oggi sotto MIT, che permette a chiunque di rivendere lo stesso codice, e se la vendita diventa concreta quella scelta va rivista con cognizione.

## ADR-026, la simulazione riceve le estrazioni, e il suo accordo con il foglio si congela in un test

Data: 2026-09-09. Stato: accettata.

Contesto. La simulazione probabilistica del foglio Rischio era l'ultima parte del modello con una sola implementazione, cioè le formule del workbook e le estrazioni congelate nel foglio nascosto. L'applicazione web ne ha bisogno nel browser, quindi una seconda implementazione andava scritta comunque, e ADR-024 vieta di scriverne una senza presidio. Il presidio esistente, cioè i vettori generati dal motore Python, presuppone però una funzione deterministica dei soli input, e una simulazione non lo è: dipende anche dalle estrazioni, che Python e JavaScript non possono produrre uguali.

Decisione. Le estrazioni sono un parametro della simulazione e non una sua parte interna, in tutte le implementazioni. La funzione che le genera con il seme dichiarato resta accanto, e riproduce seme e ordine di chiamata del generatore del workbook. I vettori di riscontro della simulazione stanno in un file proprio e portano con sé il campione di estrazioni con cui sono stati prodotti, così che il confronto fra le due implementazioni resti alla tolleranza del resto del motore, cioè un miliardesimo relativo. Il generatore di estrazioni per l'uso interattivo nel browser è dichiaratamente fuori dal confronto, e di esso si verifica il solo requisito che ha, cioè che lo stesso seme dia sempre la stessa sequenza. I valori che il foglio calcola sul caso precaricato, misurati con Excel, sono congelati in `tests/test_rischio.py`.

Motivazione. Spostare la casualità fra i parametri è ciò che rende la simulazione una funzione pura, e una funzione pura si confronta bit per bit. L'alternativa, cioè confrontare due simulazioni con generatori diversi, obbligherebbe a tolleranze statistiche larghe, ossia a una suite che passa anche quando una delle due implementazioni sbaglia: sarebbe un presidio per finta. Congelare i numeri di Excel in un test è la seconda metà della stessa scelta, perché l'accordo fra il modulo e il foglio senza un test resterebbe la fotografia di un giorno, e la divergenza fra le due arriverebbe in silenzio come già accaduto altrove in questo progetto.

Conseguenze. Le due statistiche del foglio, cioè il percentile interpolato e la mediana dei due centrali, e il suo arrotondamento, che sul mezzo si allontana da zero, sono riscritte nei moduli invece di essere prese dalle funzioni omonime dei due linguaggi: erano tre convenzioni diverse con lo stesso nome, e l'arrotondamento ha prodotto la sola divergenza vera del primo confronto, un anno di durata del mutuo e centosettanta euro di rata annua. L'evento di morosità grave si decide nella scala normale, confrontando la mescolanza con l'inversa della normale della probabilità invece di andare e tornare dalla cumulata: è la stessa disuguaglianza, attraversa una funzione in meno, e non richiede una funzione degli errori di pari precisione in ogni linguaggio. Restano dichiarate due cose che il porto ha fatto emergere e che questa decisione non risolve: la formula chiusa del debito residuo nel foglio divide per zero a tasso nullo, dove i moduli usano il limite, e la simulazione tassa il ricavo con la cedolare sul canone libero qualunque sia il regime scelto, dove i moduli portano l'aliquota come parametro con lo stesso valore predefinito.

## ADR-027, una regola che vale da due lati si scrive una volta, e dove i lati non condividono il linguaggio si genera

Data: 2026-09-14. Stato: accettata.

Contesto. Con la fase tre nasce il secondo lato dell'applicazione. Fino al 13 settembre 2026 la forma di un immobile e la regola che la valida vivevano dentro il file delle rotte del Worker, ed era corretto perché l'unico chiamante era una prova automatica. Un modulo che compila quei campi e li invia rende la stessa informazione necessaria da due parti, e con essa la gerarchia dei ruoli, che l'interfaccia deve conoscere per non offrire un'azione destinata a un rifiuto, e il catalogo delle verifiche pre-acquisto, che esisteva come lista letterale dentro il generatore del foglio Checklist.

Decisione. Le regole che due lati devono applicare stanno in `app/src/condiviso/` e le importano entrambi: la forma di un immobile con la sua validazione e i suoi valori predefiniti, e la gerarchia dei ruoli con la funzione che la interroga. Dove i due lati non condividono il linguaggio la regola resta presso la fonte e si genera verso l'altro: il catalogo delle verifiche esce dal generatore Excel, diventa `src/immobiliare/verifiche.py` e `tools/genera-motore.py` lo emette in TypeScript come quarta uscita, con lo stesso controllo di scadenza dei parametri fiscali. La difesa resta una sola e resta quella del Worker: l'applicazione della regola nel browser è cortesia, non sicurezza, e non ne sostituisce nessuna parte. Le ipotesi di valutazione, che sono un documento scritto da sei aree diverse, si scrivono conservando ogni chiave che l'area non conosce e si leggono tollerando ogni chiave che non riconosce.

Motivazione. I due versi della divergenza fra due copie di una regola non sono simmetrici. Un'interfaccia più severa del server produce un fastidio che qualcuno segnala; una più permissiva produce un modulo che dice tutto bene e un rifiuto che arriva dopo un giro di rete, su un campo che il modulo non sa evidenziare perché per lui erano tutti giusti. Sui valori predefiniti la divergenza non produce nemmeno un errore: fa nascere diversi un immobile creato dal modulo e uno creato da una chiamata diretta, e nessuno dei due è sbagliato. Sul catalogo delle verifiche produrrebbe due elenchi di verifiche legali diversi fra il foglio e l'applicazione, cioè il genere di divergenza che si scopre davanti a un notaio. Sul documento delle ipotesi produrrebbe la cancellazione silenziosa del lavoro di un'altra area, che è il carattere di guasto che ADR-013 aveva già isolato nel workbook.

Conseguenze. La cartella `app/src/condiviso/` è il terzo luogo del progetto che ha una sola fonte di verità dichiarata, dopo `parametri.py` per la fiscalità e `calcoli.py` per il modello. Il file generato `app/src/condiviso/verifiche.generate.ts` non si modifica a mano, e `python tools/genera-motore.py --check` esamina da oggi quattro file invece di tre, quindi il flusso di distribuzione lo controlla senza modifiche. La ripartizione delle prove ne discende: il contenuto di una regola condivisa si prova una volta sola in Node su una funzione pura, mentre la sua applicazione si prova dove ha una conseguenza, cioè dentro il runtime di Cloudflare contro un D1 vero. Resta vincolante che nessuna sezione dell'interfaccia contenga una regola di dominio: sta nel modulo condiviso se vale da due lati, nel modello puro dell'area se vale solo lì, e nel motore se è un calcolo.

## ADR-028, il contesto di lavoro appartiene al guscio, e un'area mostra invece di navigare

Data: 2026-09-14. Stato: accettata.

Contesto. Con la seconda delle sei aree, cioè il costo dell'operazione, si è posta per la prima volta la domanda di che cosa un'area possieda. La prima area possedeva tutto: caricava l'elenco degli immobili, teneva la selezione, chiamava il server per salvare, e leggeva e riscriveva la propria porzione del documento delle ipotesi. Era corretto per una schermata sola e diventa un pattern da ricopiare cinque volte, il che è una cosa diversa.

Decisione. Il guscio possiede il contesto di lavoro, cioè l'elenco degli immobili dell'organizzazione, l'immobile aperto e il ciclo di salvataggio; un'area riceve l'immobile, il ruolo, una funzione che salva e una che rimuove, e non riceve il cliente dell'interfaccia di programmazione. Il ciclo di modifica e salvataggio vive in un hook solo, `useBozza`, che azzera la bozza quando cambia l'immobile aperto, applica la validazione condivisa prima di partire, tiene lo stato di attesa e porta fuori il rifiuto del server. La forma dell'intero documento delle ipotesi è dichiarata in `app/src/condiviso/ipotesi.ts`, e la sola funzione che produce un documento nuovo conserva per costruzione sia le sezioni delle altre aree sia i campi sconosciuti dentro la sezione che tocca. Una sezione può avere due autori, purché la sua forma sia dichiarata in un posto solo. I valori predefiniti si generano dalle dataclass di `calcoli.py`, e i pochi che l'introspezione non vede si derivano da dove il progetto li dichiara già, con una prova che presidia la derivazione.

Motivazione. La domanda che ha deciso non è quale forma sia più elegante ma quale parte del codice della prima area, ricopiata male, fallirebbe in silenzio. Due caricamenti dell'elenco sono spreco e si vedono; una selezione persa cambiando area è un fastidio e si vede; la conservazione dimenticata cancella il lavoro di un'altra area senza errore; e l'azzeramento della bozza dimenticato mostra i dati di un immobile sotto il nome di un altro e li fa sovrascrivere. Le ultime due non producono nulla di visibile, quindi non si spostano perché siano più ordinate: si spostano perché siano l'unica strada. Non ricevere il cliente segue lo stesso criterio: ciò che un'area ha in mano finirà per usarlo, e con il cliente potrebbe rifarsi per conto proprio tutto ciò che le si è appena tolto.

Conseguenze. Il contratto `ProprietaSezione` è il punto in cui si legge che cosa una sezione può fare, e allargarlo è una decisione visibile invece di una comodità locale. `rimuovi` vale null quando non c'è un immobile aperto, e un'area che non può lavorare senza immobile si divide in un componente esterno che decide e uno interno che ne riceve uno non nullo: uno stato che non ha senso non si rappresenta con un valore neutro ma con un tipo che lo esclude. Il generatore ha da oggi cinque uscite invece di quattro, e `python tools/genera-motore.py --check` ne esamina cinque. Resta vincolante che nessuna sezione contenga una regola di dominio: sta nel modulo condiviso se vale da due lati, nel modello puro dell'area se vale solo lì, e nel motore se è un calcolo.

## ADR-029, il livello di piattaforma amministra le organizzazioni e non ne legge i dati

Data: 2026-09-14. Stato: accettata.

Contesto. Fino al 14 settembre 2026 l'applicazione conosceva tre livelli, tutti dentro un'organizzazione: amministratore, membro e lettore. Bastavano a descrivere un'agenzia e non bastavano a descrivere un prodotto venduto a più agenzie, perché lasciavano fuori due domande che qualcuno deve pur poter fare. La prima è chi crea l'organizzazione di un cliente nuovo e le dà il suo primo amministratore, e la risposta era "si inserisce a mano nel database", che va bene una volta e non va bene mai più. La seconda è chi, dentro un'agenzia, invita un collega o gli revoca l'accesso: il ruolo che lo consentirebbe esisteva dal 7 settembre, mancavano le rotte, quindi anche quella era una scrittura a mano.

Decisione. Nasce una seconda famiglia di livelli, di piattaforma e non di organizzazione, con due valori ordinati: `supporto` legge quali organizzazioni esistono e chi ne fa parte, `superamministratore` le crea, le rimuove e ne nomina gli amministratori. Le due famiglie non si sommano e non si implicano: un livello di piattaforma non concede alcun accesso agli immobili di alcuna organizzazione, e il controllo di appartenenza non ha e non avrà una scorciatoia del tipo "oppure è superamministratore". Un superamministratore che voglia vedere i dati di un cliente deve aggiungersi fra i suoi membri, e quell'aggiunta lascia una riga. Il registro delle rotte distingue le due famiglie con un requisito discriminato invece che con due campi facoltativi, così che una rotta senza requisito non compili. L'invariante che un'organizzazione non resti senza amministratori è difeso nel server e in un posto solo, perché le operazioni che possono violarlo sono due, cioè la revoca e la retrocessione.

Motivazione. La scelta di sostanza è il perimetro, non l'esistenza del livello. Su un prodotto venduto, chi lo vende ha bisogno di creare il contenitore di un cliente e non ha bisogno di leggerne le trattative, e concedere il secondo insieme al primo sarebbe comodo oggi e indifendibile il giorno in cui un cliente chiedesse chi può vedere i suoi dati. Vale però enunciare il limite senza addolcirlo: un superamministratore può aggiungersi da solo, quindi la protezione non è impossibilità ma tracciabilità. La forma che renderebbe impossibile la lettura, cioè cifrare i dati con una chiave che il fornitore non possiede, è una scelta diversa e più costosa che questo progetto non ha preso, e dichiararlo serve a che nessuno, fra sei mesi, prometta a un cliente la cosa sbagliata. I due livelli invece di uno hanno una ragione minore ma reale: un livello solo non è una gerarchia, è un valore booleano travestito, e la prima volta che servisse distinguere fra chi guarda e chi cambia andrebbe migrata la tabella.

Conseguenze. La migrazione `0002_gestori.sql` aggiunge la tabella dei livelli di piattaforma, e il passo 8 di [`messa-in-rete.md`](../../docs/messa-in-rete.md) cambia di conseguenza: resta una sola scrittura a mano nella vita del sistema, cioè il primo superamministratore, e tutto il resto si fa dal pannello. La rotta `/api/io` riporta anche il livello, perché l'interfaccia deve sapere se mostrare il pannello prima di poterlo chiedere, e una rotta che rispondesse "non ti è permesso" costringerebbe a trattare un rifiuto come esito ordinario. Il guscio lascia passare chi ha un livello anche senza appartenenze, altrimenti il primo superamministratore sarebbe fermato dalla schermata che dice che non appartiene a nulla e non raggiungerebbe mai il pannello con cui crearsi un'organizzazione. La schermata di amministrazione ha un contratto proprio e non riusa quello delle sei aree, perché allargare quello avrebbe consegnato il cliente dell'interfaccia di programmazione anche a esse, cioè disfatto ADR-028.
