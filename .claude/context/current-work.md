...
- [x] Fase uno: motore in TypeScript, generatore di vettori dal motore Python, suite che li verifica. Duecentoundici casi, tolleranza 1e-9 relativo dichiarata prima, tutti passati, più un caso che deve fallire
- [x] Fase uno, ricaduta: corretto il difetto del motore Python che il generatore ha fatto emergere, cioè la divisione per zero nel tasso interno sui flussi mensili
...
- [x] Fase due, procedura: [`messa-in-rete.md`](../../docs/messa-in-rete.md) in nove passi, ciascuno con la ragione, l'azione esatta, che cosa riportare e come si verifica, più il registro dell'avanzamento con le date
...
- [x] Fase tre, catena di costruzione: interfaccia React con TypeScript costruita da Vite verso `app/statico/`, sezione delle risorse statiche in `wrangler.toml` con il non trovato che passa al Worker, e il ripiego della pagina singola scritto nel Worker perché non intercetti anche `/api`
- [x] Fase tre, regole condivise: `app/src/condiviso/` con la forma dell'immobile, la sua validazione, i suoi predefiniti e la gerarchia dei ruoli, importati dal Worker e dal browser, per ADR-027
- [x] Fase tre, catalogo delle verifiche: estratto dal generatore Excel in `src/immobiliare/verifiche.py` ed emesso in TypeScript da `tools/genera-motore.py`, quarta uscita del presidio con il suo controllo di scadenza
...
- [x] Livelli di accesso: quattro, cioè lettore, membro e amministratore dentro un'organizzazione, più il livello di piattaforma con supporto e superamministratore, per ADR-029; rotte per i membri e per le organizzazioni, invariante dell'ultimo amministratore, e la schermata di amministrazione con i suoi due pannelli
- [x] Presidio del flusso corretto: il controllo sul segnaposto del database è passato dalle prove al passo che distribuisce, dove suona una volta sola invece che a ogni spinta
...
Nel foglio Confronto immobili restano globali l'opzione prezzo-valore e la qualifica di immobile di lusso, prese dal foglio Immobile. Dal 14 settembre 2026 il limite vale per il solo workbook: nell'applicazione web i quattro valori del regime di acquisto, cioè prima casa, prezzo-valore, venditore impresa e nuova costruzione, sono attributi di ciascun immobile, e l'area immobile li dichiara uno per uno. Portarle nel registro sarebbe meccanicamente identico a quanto fatto per prima casa e venditore impresa, e non è stato fatto perché la prima è una scelta che conviene quasi sempre e la seconda riguarda un caso raro: se in lista compare un immobile in categoria A/1, A/8 o A/9, va valutato a parte.
