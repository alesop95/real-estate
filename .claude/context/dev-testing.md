...
Il terzo livello sono i test automatici, in tre file. `tests/test_calcoli.py` copre il motore di dominio e congela il caso di riferimento. `tests/test_workbook.py` copre la struttura del workbook generato: l'elenco dei fogli, la presenza dei nomi definiti essenziali, e soprattutto la corrispondenza posizionale fra le colonne del foglio Annunci e l'ordine con cui `annunci.esporta_in_excel` le scrive, che è il contratto più fragile del progetto perché vive in due file diversi e nessun tipo lo protegge. Quel test esporta un annuncio noto e rilegge le celle una per una, e ha già ripagato il costo trovando un difetto reale: `openpyxl` ignora l'assegnazione quando si passa `value=None` a `cell()`, quindi un campo azzerato non ripuliva la cella e l'annuncio esportato ereditava in silenzio il dato di quello che occupava prima quella riga.

`tests/test_rischio.py`, dal 9 settembre 2026, copre la simulazione probabilistica in due modi distinti: congela le proprietà dichiarate in ADR-023, che sono la specifica del fattore comune, e congela i numeri che Excel calcola sul foglio Rischio, che sono la prova dell'accordo fra le due implementazioni. La seconda famiglia è la più preziosa e la più insolita del progetto, perché mette in un test il risultato di una misura fatta con un altro programma: se qualcuno cambia il modulo Python senza cambiare il foglio, o viceversa, il test lo dice il giorno stesso invece di lasciare che le due versioni della verità coesistano.

Al 9 settembre 2026 i test sono centosette, cinquantadue sul motore di calcolo e sui moduli di dominio, ventisei sulla struttura del workbook, sull'acquisizione e sulla graduatoria, e ventinove sulla simulazione del rischio.
...
La prova del 9 settembre 2026 ha una forma diversa dalle precedenti e conviene descriverla, perché è riusabile ogni volta che una parte del foglio viene portata in Python. Invece di scrivere input e leggere un esito, legge tutto: le celle con nome definito che alimentano il foglio Rischio, la tabella dei percentili, le quattro probabilità e le ventiquattro celle del tornado, più le prime righe del foglio nascosto delle estrazioni, e le scrive in un file JSON. Un secondo script costruisce la base della simulazione da quei nomi ed esegue il modulo Python sugli stessi mille numeri estratti, poi confronta voce per voce. Il vantaggio è che il confronto non dipende dalla lettura a occhio di una schermata e produce un unico numero da guardare, cioè lo scarto relativo massimo, che quel giorno valeva 1,5 su dieci alla tredicesima dopo aver corretto la sola divergenza vera, cioè l'arrotondamento del mezzo. I due script vivono fuori dal repository perché servono una volta per porto, mentre i valori che hanno prodotto restano dentro, congelati in `tests/test_rischio.py`.
...
Dopo ogni modifica alla simulazione del rischio, cioè a `src/immobiliare/rischio.py` o alle formule del foglio Rischio e del foglio nascosto delle estrazioni, si rigenerano i vettori e si guarda se i valori congelati in `tests/test_rischio.py` reggono ancora. Se non reggono la domanda da farsi è quale delle due parti si è mossa: un cambio voluto del modello va portato in entrambe e i valori congelati vanno rimisurati con Excel, mentre un cambio non voluto è un difetto e il test ha fatto il suo lavoro.
